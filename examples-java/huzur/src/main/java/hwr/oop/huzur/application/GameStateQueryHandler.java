...
public final class GameStateQueryHandler implements GameStateQuery {
  ...
  public GameStateDto gameStateOf(String gameId) {
    ...
    return new GameStateDto(turn.id(),
        game.handOf(turn).cards().map(Card::shortHandle).toList(),
        layoutOptional
            .map(layout -> layout.cards().map(Card::shortHandle).toList())
            .orElse(Collections.emptyList()),
        layoutOptional
            .map(layout -> layout.allCards().map(Card::shortHandle).toList())
            .orElse(Collections.emptyList()),
        layoutOptional.map(Layout::finishedOnPlay).orElse(false),
        game.players().collect(Collectors.toMap(Player::id, p -> game.handOf(p).numberOfCards())),
