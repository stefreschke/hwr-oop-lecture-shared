...
final class AnswerLayout implements Layout {
  ...
  private void assertCorrectNumberOfCards() {
    ...
    if (cardNumber < numberOfCards) {
      throw new IllegalArgumentException(String.format("not enough cards, required %d, got %d",
          numberOfCards,
          cardNumber
      ));
    }
    if (cardNumber > numberOfCards) {
      throw new IllegalArgumentException(String.format("too many cards, required %d, got %d",
          numberOfCards,
          cardNumber
      ));
    }
  ...
  private void assertCardsStrongEnough(Layout previous, Comparator<Card> comparator) {
    ...
    strengthMap.forEach((card, strongerCards) -> {
      ...
      if (remainingVs.isEmpty()) {
        ...
        throw new IllegalArgumentException(String.format(
            "not strong enough cards," + " can't use any of %s on %s", notUsefulCards, card));
      } else {
  ...
  private Map<Card, List<Card>> buildStrengthMap(List<Card> cards, Layout previous, Comparator<Card> comparator) {
    final var previousCards = previous.cards().toList();
    return previousCards
        .stream()
        .collect(Collectors.toMap(Function.identity(),
            p -> cards.stream().filter(c -> comparator.compare(p, c) < 0).sorted(comparator).toList()
        ));
  }
